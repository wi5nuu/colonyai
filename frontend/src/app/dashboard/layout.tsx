...
import {
  ...
  Lock, ChevronLeft, ChevronRight, CheckCircle, AlertCircle, Globe
} from 'lucide-react'
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [notifications, setNotifications] = useState<{ id: string | number; type: string; title: string; message: string; time: string; read: boolean; }[]>([])
  ...
  return (
    ...
    <div className="h-screen bg-[#f4f7f6] flex font-sans selection:bg-primary/20 selection:text-primary overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 ${isCollapsed ? 'lg:w-14' : 'lg:w-48'} w-48 bg-white border-r border-slate-100 transform transition-all duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} rounded-xl lg:rounded-none lg:mr-0 shadow-2xl lg:shadow-none`}>
        <div className="flex flex-col h-full text-slate-600">
          <div className={`flex items-center justify-between pt-3 pb-3 px-4 transition-all duration-300 ${isCollapsed ? 'px-3 justify-center' : ''}`}>
            <div className="flex items-center gap-3">
              <div className={`flex items-center justify-center transition-all drop-shadow-sm ${isCollapsed ? 'w-10 h-10' : 'w-10 h-10'}`}>
                <img src="/android-chrome-512x512.png" alt="ColonyAI Logo" className="w-full h-full object-contain" />
              </div>
              {!isCollapsed && (
                <div className="flex flex-col animate-in fade-in duration-300">
                  <span className="text-base font-bold tracking-tight text-slate-900">ColonyAI</span>
                  {user?.role === 'super_admin' && (
                    <span className="text-[8px] font-bold bg-primary/10 text-primary px-1.5 py-0.5 rounded uppercase tracking-[0.2em] w-fit">Nexus Master</span>
                  )}
                </div>
              )}
            </div>
              ...
              .map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <item.icon className={`transition-all ${isCollapsed ? 'h-5 w-5' : 'h-3.5 w-3.5'}`} />
                    {!isCollapsed && <span className="text-[10px] font-bold tracking-wide animate-in fade-in slide-in-from-left-2 duration-300">{t(item.tKey)}</span>}
                  </Link>
                )
            })}
          </nav>
          ...
          <div className={`py-6 mt-auto transition-all duration-300 ${isCollapsed ? 'px-0' : 'px-6'}`}>
            {!isCollapsed && (
              <div className="bg-slate-50 rounded-xl p-4 relative overflow-hidden group border border-slate-100 animate-in fade-in zoom-in duration-300">
                <div className="relative z-10">
                  <p className="text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-widest">{t('header.nextCalibration')}</p>
            ...
            >
              ...
              {!isCollapsed && <span className="text-[10px] font-bold">{t('nav.logout')}</span>}
            </button>
              ...
              {notificationsOpen && (
                <div className="absolute right-0 mt-3 w-72 sm:w-80 bg-white rounded-xl shadow-2xl border border-slate-100 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="px-4 py-3 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                    <h3 className="text-xs font-bold text-slate-900 uppercase tracking-widest">{t('header.notifications')}</h3>
                    <button
                      onClick={() => {
                  ...
                  <div className="max-h-[300px] overflow-y-auto">
                    {notifications.length > 0 ? (
                      ...
                      <div className="p-8 text-center">
                        ...
                        <p className="text-xs font-bold text-slate-300 uppercase tracking-widest">{t('header.noNewSignals')}</p>
                      </div>
                    )}
                  </div>
