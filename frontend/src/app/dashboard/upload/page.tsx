...
export default function UploadPage() {
  ...
  const handleFile = (file: File) => {
    if (!file.type.startsWith('image/')) { toast.error(t('upload.errorImageFile')); return }
    if (file.size > 10 * 1024 * 1024) { toast.error(t('upload.errorFileSize')); return }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedFile) { toast.error(t('upload.errorSelectImage')); return }
    if (!formData.sampleId.trim()) { toast.error(t('upload.errorSampleId')); return }
    ...
    try {
      ...
      toast.dismiss()
      toast.success(`${t('upload.analysisComplete')}: ${analysis.colony_count} ${t('upload.coloniesDetected')}`)
      router.push(`/dashboard/results/${analysis.id}`)
    } catch (error: any) {
      toast.dismiss()
      toast.error(error.response?.data?.detail || error.message || t('upload.analysisFailed'))
    } finally {
      ...
      <div className="flex relative min-h-[calc(100vh-200px)]">
        ...
        <div className={`flex-1 transition-all duration-300 ${showDocs ? 'lg:mr-[350px]' : ''}`}>
          <div className="max-w-[1500px] mx-auto px-8 py-8">
            ...
            <div className="mb-10">
              <div className="flex items-center justify-between">
                 <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">{t('upload.title')}</h1>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1">{t('upload.subtitle')}</p>
                    
                    <DocumentationToggle showDocs={showDocs} setShowDocs={setShowDocs} text={t('upload.docsToggle')} />                 </div>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900">{t('upload.plateImage')}</h2>
              <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">{t('upload.imageRules')}</p>
            </div>
          </div>
            ...
            {preview ? (
              <div className="relative group/preview w-full h-full">
                ...
                <div className="absolute inset-0 bg-primary/40 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover/preview:opacity-100 transition-all duration-300">
                  ...
                  >
                    <Trash2 className="h-3.5 w-3.5" /> {t('upload.removeImage')}
                  </button>
                </div>
              ...
              <div className="flex flex-col items-center justify-center p-8 text-center">
                ...
                <label htmlFor="file-upload" className="cursor-pointer">
                  ...
                  <span className="text-base text-slate-500 font-medium"> {t('upload.dragAndDrop')}</span>
                  <input id="file-upload" name="file-upload" type="file" className="sr-only" accept="image/*" onChange={handleChange} />
                </label>
                <p className="text-[8px] text-slate-400 mt-2 font-black uppercase tracking-widest">{t('upload.isoPreferred')}</p>
              </div>
            )}
          </div>
        ...
        <div className="dashboard-card p-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900">{t('upload.biologicalProtocol')}</h2>
              <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">{t('upload.parametersConfig')}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-1.5">
              <label htmlFor="sampleId" className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
                {t('upload.specimenIdentifier')} <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
                id="sampleId"
            ...
            <div className="space-y-1">
              <label htmlFor="mediaType" className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
                {t('upload.agarMediaMatrix')} <span className="text-rose-500">*</span>
              </label>
              <div className="relative">
                ...
                >
                  ...
                  <option value="Other">{t('upload.otherProtocol')}</option>
                </select>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label htmlFor="dilutionFactor" className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
                  {t('upload.dilutionFactor')} <span className="text-rose-500">*</span>
                </label>
                <div className="relative">
                  <select
                    id="dilutionFactor"
              ...
              <div className="space-y-1">
                <label htmlFor="platedVolume" className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
                  {t('upload.volume')} <span className="text-rose-500">*</span>
                </label>
                <input
                  type="number"
                  id="platedVolume"
